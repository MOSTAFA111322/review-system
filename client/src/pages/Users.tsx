...
import { History, KeyRound, MonitorSmartphone, Pencil, ShieldCheck, UserPlus, UsersRound } from "lucide-react";
...
export default function Users() {
  ...
  const renameLocalUsername = trpc.users.renameLocalUsername.useMutation({ onSuccess: async result => { setFeedback(result.changed ? "تم تعديل اسم المستخدم المحلي وإبطال الجلسات السابقة. استخدم الاسم الجديد عند الدخول القادم." : "اسم المستخدم لم يتغير."); await utils.users.list.invalidate(); } });
  ...
  <Card className="overflow-hidden rounded-2xl"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><UsersRound className="h-4 w-4 text-blue-700" />الحسابات</CardTitle><CardDescription>يُتاح تعطيل الحساب وتعديل اسم المستخدم وإعادة تعيين كلمة المرور للحسابات المحلية فقط. يؤدي التعطيل أو تعديل بيانات الدخول إلى إبطال الجلسات السابقة.</CardDescription></CardHeader><CardContent className="p-0">{users.isLoading ? <p className="p-6 text-center text-sm text-slate-500">جارٍ تحميل الحسابات…</p> : users.data?.length ? <div className="divide-y divide-slate-100">{users.data.map(account => <AccountRow key={account.id} account={account} onToggle={() => setActive.mutate({ userId: account.id, isActive: !account.isActive })} onReset={nextPassword => resetPassword.mutate({ userId: account.id, password: nextPassword })} onRename={nextUsername => renameLocalUsername.mutate({ userId: account.id, username: nextUsername })} renamePending={renameLocalUsername.isPending} renameError={renameLocalUsername.error?.message ?? null} />)}</div> : <p className="p-6 text-center text-sm text-slate-500">لا توجد حسابات بعد.</p>}</CardContent></Card>
...
function AccountRow({ account, onToggle, onReset, onRename, renamePending, renameError }: { account: { id: number; name: string | null; username: string | null; loginMethod: string | null; hasLocalPassword: boolean; isActive: boolean; roles: { name: string }[] }; onToggle: () => void; onReset: (password: string) => void; onRename: (username: string) => void; renamePending: boolean; renameError: string | null }) {
  const [editingPassword, setEditingPassword] = useState(false);
  const [nextPassword, setNextPassword] = useState("");
  const [editingUsername, setEditingUsername] = useState(false);
  const [nextUsername, setNextUsername] = useState(account.username ?? "");
  const [usernameError, setUsernameError] = useState<string | null>(null);
  const submitUsername = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const value = nextUsername.trim();
    if (!localUsernamePattern.test(value)) { setUsernameError(usernameGuidance); return; }
    setUsernameError(null);
    onRename(value);
  };
  return <div className="px-5 py-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-slate-800">{account.name || "مستخدم النظام"}</p><p dir="auto" className="mt-1 text-xs text-slate-500">{account.username || (account.loginMethod === "oauth" ? "حساب مانوس" : "—")}</p><div className="mt-2 flex flex-wrap gap-1.5">{account.roles.map(role => <Badge key={role.name} variant="secondary">{role.name}</Badge>)}{account.hasLocalPassword ? <Badge className="border-0 bg-blue-50 text-blue-700">حساب محلي</Badge> : <Badge className="border-0 bg-slate-100 text-slate-600">حساب مانوس</Badge>}</div></div><div className="flex flex-wrap items-center gap-2"><Badge className={account.isActive ? "border-0 bg-emerald-50 text-emerald-700" : "border-0 bg-red-50 text-red-700"}>{account.isActive ? "نشط" : "معطل"}</Badge><Button type="button" variant="outline" size="sm" onClick={onToggle}>{account.isActive ? "تعطيل" : "تفعيل"}</Button>{account.hasLocalPassword ? <><Button type="button" variant="outline" size="sm" onClick={() => { setEditingUsername(value => !value); setUsernameError(null); setNextUsername(account.username ?? ""); }}><Pencil className="h-3.5 w-3.5" />تعديل الاسم</Button><Button type="button" variant="outline" size="sm" onClick={() => setEditingPassword(value => !value)}><KeyRound className="h-3.5 w-3.5" />إعادة تعيين</Button></> : null}</div></div>{editingUsername ? <form className="mt-4 grid gap-2 border-t border-slate-100 pt-4 sm:grid-cols-[minmax(0,1fr)_auto_auto]" onSubmit={submitUsername}><Input aria-label={`اسم المستخدم الجديد لـ ${account.name || account.username}`} dir="auto" value={nextUsername} onChange={event => { setNextUsername(event.target.value); setUsernameError(null); }} placeholder="اسم المستخدم الجديد" required /><Button type="submit" size="sm" disabled={renamePending}>{renamePending ? "جارٍ الحفظ…" : "حفظ الاسم"}</Button><Button type="button" variant="outline" size="sm" onClick={() => setEditingUsername(false)}>إلغاء</Button><p className="text-xs leading-5 text-slate-500 sm:col-span-3">{usernameGuidance} سيُطلب من المستخدم الدخول بالاسم الجديد بعد حفظه.</p>{usernameError || renameError ? <p role="alert" className="text-xs leading-5 text-red-600 sm:col-span-3">{usernameError || describeLocalAccountError(renameError ?? "")}</p> : null}</form> : null}{editingPassword ? <form className="mt-4 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row" onSubmit={event => { event.preventDefault(); onReset(nextPassword); setNextPassword(""); setEditingPassword(false); }}><Input aria-label={`كلمة المرور الجديدة لـ ${account.name || account.username}`} type="password" dir="ltr" minLength={8} value={nextPassword} onChange={event => setNextPassword(event.target.value)} placeholder="كلمة مرور جديدة، 8 أحرف على الأقل" required /><Button type="submit" size="sm">حفظ كلمة المرور</Button></form> : null}</div>;
}
