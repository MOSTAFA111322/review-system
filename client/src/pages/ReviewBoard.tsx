...
import { AlertCircle, Archive, ArrowLeft, Ban, CalendarClock, ChevronLeft, ChevronRight, CirclePlus, ClipboardList, Filter, Search, SlidersHorizontal, TriangleAlert, UserRoundX } from "lucide-react";
...
export default function ReviewBoard({ fiscalYearId, fiscalYearName, isClosed }: Props) {
  ...
  const [attention, setAttention] = useState<"all" | "overdue" | "dueSoon" | "unassigned" | "critical">("all");
  ...
  return (
    <section dir="rtl" className="mx-auto max-w-[1500px] space-y-5">
      ...
      {archiveScope === "active" ? <div className="rounded-2xl border border-blue-100 bg-gradient-to-l from-blue-50/70 to-white p-4 shadow-sm dark:border-blue-900/70 dark:from-blue-950/35 dark:to-slate-950"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-slate-100"><TriangleAlert className="h-4 w-4 text-orange-600" />تحتاج متابعة</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-300">مرشحات سريعة تُطبّق من الخادم ضمن صلاحياتك وسنتك المالية.</p></div><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant={attention === "overdue" ? "default" : "outline"} onClick={() => { setPage(1); setAttention(attention === "overdue" ? "all" : "overdue"); }} className={attention === "overdue" ? "gap-1.5 bg-blue-700 hover:bg-blue-800" : "gap-1.5 border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"}><TriangleAlert className="h-3.5 w-3.5" />متأخرة</Button><Button type="button" size="sm" variant={attention === "dueSoon" ? "default" : "outline"} onClick={() => { setPage(1); setAttention(attention === "dueSoon" ? "all" : "dueSoon"); }} className={attention === "dueSoon" ? "gap-1.5 bg-blue-700 hover:bg-blue-800" : "gap-1.5 border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"}><CalendarClock className="h-3.5 w-3.5" />خلال 3 أيام</Button><Button type="button" size="sm" variant={attention === "unassigned" ? "default" : "outline"} onClick={() => { setPage(1); setAttention(attention === "unassigned" ? "all" : "unassigned"); }} className={attention === "unassigned" ? "gap-1.5 bg-blue-700 hover:bg-blue-800" : "gap-1.5 border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"}><UserRoundX className="h-3.5 w-3.5" />دون تكليف</Button><Button type="button" size="sm" variant={attention === "critical" ? "default" : "outline"} onClick={() => { setPage(1); setAttention(attention === "critical" ? "all" : "critical"); }} className={attention === "critical" ? "gap-1.5 bg-blue-700 hover:bg-blue-800" : "gap-1.5 border-slate-200 bg-white text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"}><AlertCircle className="h-3.5 w-3.5" />عاجلة</Button></div></div></div> : null}
      ...
      <details className="group rounded-2xl border border-slate-200 bg-white px-4 shadow-sm dark:border-slate-700 dark:bg-slate-900"><summary className="flex cursor-pointer list-none items-center justify-between py-4 text-sm font-bold text-slate-800 dark:text-slate-100"><span>دليل المسار السريع للعمل</span><span className="text-xs font-medium text-blue-700 group-open:hidden dark:text-blue-300">عرض الدليل</span><span className="hidden text-xs font-medium text-blue-700 group-open:inline dark:text-blue-300">إخفاء الدليل</span></summary><div className="grid gap-3 border-t border-slate-100 py-4 text-sm dark:border-slate-800 md:grid-cols-4"><GuideStep number="1" title="حدّد التكليف" text="أكمل بيانات العملية وأسندها للموظف المسؤول." /><GuideStep number="2" title="تابع الحالتين" text="لا تُعد العملية مكتملة حتى يكتمل مسارا الموظف والمراجع." /><GuideStep number="3" title="أرشفة أو إلغاء" text="أرشف المكتمل، وألغِ غير الصالح مع سبب واضح. لا يُحذف السجل." /><GuideStep number="4" title="راجع الاستثناءات" text="استخدم مرشحات المتابعة للمتأخر والعاجل وغير المكلف." /></div></details>
...
function GuideStep({ number, title, text }: { number: string; title: string; text: string }) { return <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/70"><span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-blue-700 text-[11px] font-bold text-white">{number}</span><p className="mt-2 font-bold text-slate-800 dark:text-slate-100">{title}</p><p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-300">{text}</p></div>; }
function DueHint({ dueDate }: { dueDate: string | Date | null }) { if (!dueDate) return null; const due = new Date(dueDate instanceof Date ? dueDate.getTime() : dueDate); if (Number.isNaN(due.getTime())) return null; due.setHours(0, 0, 0, 0); const today = new Date(); today.setHours(0, 0, 0, 0); const days = Math.round((due.getTime() - today.getTime()) / 86_400_000); const dayLabel = (count: number) => count === 1 ? "يوم" : count === 2 ? "يومين" : `${count} أيام`; const meta = days < 0 ? { label: `متأخرة ${dayLabel(Math.abs(days))}`, className: "text-red-700 dark:text-red-300" } : days === 0 ? { label: "اليوم", className: "text-orange-700 dark:text-orange-300" } : days <= 3 ? { label: `خلال ${dayLabel(days)}`, className: "text-amber-700 dark:text-amber-300" } : null; return meta ? <span className={`mt-1 inline-block text-[11px] font-bold ${meta.className}`}>{meta.label}</span> : null; }
