...
describe("review list input", () => {
  it("يقبل معاملات البحث والفلاتر المركبة والفرز والترقيم ضمن الحدود المعلنة", () => {
    ...
    expect(result).toMatchObject({ fiscalYearId: 1, page: 2, pageSize: 50, attention: "overdue", archiveScope: "archived", sortBy: "dueDate", sortDirection: "asc" });
  ...
  it("يرفض النطاقات غير الصالحة التي قد تتجاوز حدود قائمة المراجعات", () => {
    ...
    expect(() => reviewListInput.parse({ fiscalYearId: 1, attention: "later" })).toThrow();
  ...
  it("يبني خطة الفلترة والترقيم والفرز ويهرب رموز البحث الخاصة", () => {
    ...
    expect(plan).toMatchObject({ fiscalYearId: 9, offset: 50, queryTerm: "%100\\%\\_%", attention: "dueSoon", archiveScope: "archived", sortBy: "priority", sortDirection: "asc" });
